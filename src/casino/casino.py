...
# gets input from user on what they would like to bet on
    def chooseBet(self):
        startBet = True
        while startBet:
            betSelection = str(input("What number would you like to bet on?\n\t(N) Single Number\n\t(H) Front half or Back half\n\t(O) Odd or Even\n\t(R) Red or Black\n"))
...
# actually plays the game and uses the variable set in chooseBet to compare to actual outcome
    def playGame(self): # having an issue where bet types are overriding the check and making certain bets not calculate correctly
        ...
        while keepPlaying: # sets game to be continuous until player says to stop playing
            if self.playerBalance > 0: # checks player has any money to play with
                ...
                if whatToDo.upper() == "C":
                    print("Your current balance is $" + str(self.playerBalance))  # displays current balance

                elif whatToDo.upper() == "P":  # starts playing the game
                    betAmount = self.placeBet(self.playerBalance) # gets bets from player
                    bet = self.chooseBet()
                    if bet == "N":
    ...
    def oddOrEvenPlay(self, betAmount):
        userSelection = input("Would you like to play odd or even?\n\t(D) Odd\n\t(E) Even\n")
        spunNumber = self.spinTheWheel()
    ...
    def winText(self, num):
        return print("\tWinner winner chicken dinner! The number was " + str(num) + "!\n")
